...
def songtext(url, start_point, end_point):
    '''Takes a url, the begin point of the lyrics, and the end point as input, and 
	returns just the song text'''
    from urllib2 import urlopen
    target = urlopen(url)
    whole_page = target.read()
    text_start = whole_page.find(start_point)
    text_end = whole_page.find(end_point)
